...
const { tags, loading } = storeToRefs(darknetTagStore)
const {
  fetchTags,
  addTag,
  updateTag,
...
const allGroups = computed(() => {
  ...
  tags.value.forEach(t => t.group && set.add(t.group))
...
function onSearch() { fetchTags(searchForm.value) }
function onReset() { searchForm.value = { keyword: '', group: '', status: '' }; fetchTags() }
...
async function submitTag() {
  ...
  if (dialogType.value === 'add') {
    await addTag({ ...dialogForm.value })
    ElMessage.success('新增成功！')
  } else {
    await updateTag({ ...dialogForm.value })
...
function onBatchDisable() {
  if (!selectedRows.value.length) return ElMessage.warning('请先勾选要禁用的标签')
  batchDisableTags(selectedRows.value.map(t => t.id)).then(() => {
...
function onBatchDelete() {
  ...
  ElMessageBox.confirm('确定批量删除已选标签？', '警告', { type: 'warning' }).then(async () => {
    await batchRemoveTags(selectedRows.value.map(t => t.id))
