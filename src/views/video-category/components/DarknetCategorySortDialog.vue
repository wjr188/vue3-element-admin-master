<template>
  <el-dialog v-model="visible" title="暗网分类排序管理" width="480px" @close="handleClose" append-to-body>
    <el-form>
      ...
      <div v-if="selectedParentId && childCategoriesForSort.length">
      ...
      <div v-else-if="selectedParentId" style="text-align:center;color:#aaa;padding:24px 0">该主分类下暂无子分类</div>
...
function onSave() {
  ...
  // 提取需要保存的 id 和 sort
  const payload = sortList.value.map(item => ({
    id: item.id,
