<template>
  <div class="audio-novel-tag-manage-page">
    <!-- 顶部筛选/批量操作区 -->
    <el-card class="search-card">
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <!-- 如果有声小说标签有作者、分类等关联筛选，可在此处添加 -->
        <!-- <el-form-item label="所属作者">
          <el-select v-model="searchForm.authorId" placeholder="全部作者" clearable style="width: 130px;">
    ...
    <el-card class="table-card">
      <el-table
        :data="audioNovelTagList"
        v-loading="audioNovelTagLoading"
        border
        stripe
        class="custom-table"
        style="width: 100%;"
...
// 标签列表与 loading
const audioNovelTagList = computed(() => tagStore.list)
const audioNovelTagLoading = computed(() => tagStore.loading)
...
async function fetchTableData() {
  await tagStore.fetchList(searchForm.value)
}
...
async function onBatchDelete() {
  ...
  await ElMessageBox.confirm('确定批量删除已选标签吗？此操作将同时解除关联有声小说作品！', '警告', { type: 'warning' }).then(async () => {
    const ids = selectedRows.value.map(row => row.id)
    const res = await tagStore.batchDelete(ids)
    if (res && res.code === 0) {
...
async function onBatchSetStatus(status: number) {
  ...
  await ElMessageBox.confirm(`确定批量${actionText}已选标签吗？`, '提示', { type: 'warning' }).then(async () => {
    const ids = selectedRows.value.map(row => row.id)
    const res = await tagStore.batchSetStatus(ids, status)
    if (res && res.code === 0) {
...
async function submitDialog() {
  ...
  try {
    ...
    if (dialogType.value === 'add') {
      res = await tagStore.add(dialogForm.value)
    } else {
      res = await tagStore.update(dialogForm.value)
    }
...
async function onDelete(row: any) {
  await ElMessageBox.confirm(`确定删除标签 “${row.name}” 吗？此操作将同时解除关联有声小说作品！`, '警告', { type: 'warning' }).then(async () => {
    const res = await tagStore.remove(row.id)
    if (res && res.code === 0) {
...
<style scoped>
.audio-novel-tag-manage-page { padding: 18px; }
